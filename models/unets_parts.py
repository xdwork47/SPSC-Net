...
class deformable_LKA(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.odconv1 = ODConv2d(in_planes=dim, out_planes=dim, kernel_size=3,padding=1,groups=dim)
        self.odconv2 = ODConv2d(in_planes=dim, out_planes=dim, kernel_size=5, stride=1, padding=4, groups=dim, dilation=2)
    def forward(self, x):
        u = x.clone()
        attn = self.odconv1(x)
        attn = self.odconv2(attn)
        return u * attn
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleConv(in_channels, out_channels)
        self.ODConv = ODConv2d(in_planes=out_channels, out_planes=out_channels, kernel_size=1, padding=0, groups=out_channels)
